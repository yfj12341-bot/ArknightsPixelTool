...
- **多种速度档位**：非常快 / 快 / 中等 / 慢四档可选，适应不同设备与延迟
...
- **速度档位**：非常快 / 快 / 中等 / 慢四档，可根据实际情况选择，避免过快产生误触

## 版本历史

- **v0.2.3**：优化自动填充框选校准逻辑；修复旋转屏幕导致应用重置的问题
- **v0.2.2**：自动填充 UI 优化——进度悬浮窗支持拖动，避免遮挡画布与调色盘
- **v0.2.1**：优化鸿蒙（HarmonyOS）兼容性（v1+v2 签名）；填充开启前增加 5 秒等待与全屏倒计时
- **v0.2.0**：新增自动填充到游戏（框选画布与调色盘、四档速度档位）
